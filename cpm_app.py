    ...
    with pdfplumber.open(path) as pdf:
        for page in pdf.pages:
            ...
            if not tables:
                continue
            for tbl in tables:
                # Skip tables that are "Custom Criteria" summary sections
                # Custom criteria tables are 2-column tables with criteria IDs in the header
                # Examples:
                #   ['Custom Criteria', 'Criteria ID']
                #   ['PCSK91 6431-A Custom Supplemental Specialty PA', '6431-A']
                #   ['Rezdiffra', 'C28048-A']
                # Detail tables are 3-column tables with IDs in data rows:
                #   ['Therapeutic Class', 'Criteria Name', 'Criteria ID']
                if tbl and len(tbl) > 0:
                    first_row = tbl[0]
                    if first_row:
                        # Count non-empty columns
                        non_empty_cols = [cell for cell in first_row if cell]
                        first_row_text = ' '.join([str(cell) for cell in first_row if cell]).upper()
                        
                        # Skip 2-column tables that contain "CUSTOM" or have an ID in the header
                        if len(non_empty_cols) == 2:
                            # Skip if contains "CUSTOM CRITERIA" or "CUSTOM" in header
                            if 'CUSTOM' in first_row_text:
                                continue
                            # Skip if last column in header matches an ID pattern (like "C28048-A")
                            last_col = str(non_empty_cols[-1]).strip().upper()
                            if re.match(r'^[C]?\d{4,5}-[A-Z]$', last_col):
                                continue
